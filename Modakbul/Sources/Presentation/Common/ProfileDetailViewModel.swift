...
final class ProfileDetailViewModel: ObservableObject {
    ...
    private let blockSubject = PassthroughSubject<[(blockId: Int64, blockedUser: User)], Never>()
    ...
    private func subscribe() {
        ...
        blockSubject
            ...
            .sink { [weak self] block in
                self?.isBlocked = block.contains(where: { $0.blockedUser.id == self?.presentedUser?.id })
            }
            .store(in: &cancellables)
...
extension ProfileDetailViewModel {
    ...
    private func fetchIsBlocked(from userId: Int64, about opponentUserId: Int64) async {
        do {
            ...
            blockSubject.send(blockedUsers)
        } catch {
            blockSubject.send([])
            print(error)
        }
